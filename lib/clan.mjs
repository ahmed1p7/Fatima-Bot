...
// ═══════════════════════════════════════════════════════════════════════════════
// ✅ دالة التحقق من قائد الكلان - إصلاح مشكلة git pull
// ═══════════════════════════════════════════════════════════════════════════════

export const isClanLeader = (clan, senderId) => {
  if (!clan || !senderId) return false;
  
  // تحويل المعرفات إلى صيغة رقمية فقط للمقارنة
  const getNumericId = (id) => {
    if (!id) return '';
    const str = String(id);
    // إزالة @s.whatsapp.net أو أي لاحقة أخرى
    return str.split('@')[0].trim();
  };
  
  const senderNum = getNumericId(senderId);
  const leaderNum = getNumericId(clan.leader);
  
  // مقارنة الأرقام فقط
  return senderNum === leaderNum && senderNum !== '';
};
