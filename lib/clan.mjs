...
🏰 الكلان المتحدي: ${challenger.name}
...
⚔️ الكلان المتحدي: ${challenger.name}
⭐ المستوى: ${challenger.level}
👥 الأعضاء: ${challenger.memberCount}
⚔️ الانتصارات: ${challenger.wins}
...
❌ للرفض: .رفض_التحدي ${challengeId}`
    ...
    try {
      const channelJid = '0029VbCbgwIKgsNxh9vKb01n@s.whatsapp.net';
      await sock.sendMessage(channelJid, {
        text: `⚔️ تحدي كلان جديد! ⚔️

🏰 ${challenger.name} يتحدى 🏰 ${target.name}
...
export const acceptChallenge = async (challengeId, targetLeader, sock) => {
  ...
  setTimeout(() => endWar(challengeId, null), 30 * 60 * 1000);
