...
export const WAR_CHANNEL = "120363408713799197@newsletter";
...
export const startWarDirectly = async (clan1, clan2, sock) => {
  const rpgData = getRpgData();
  ...
  if (clan1.id === clan2.id) {
    return { success: false, message: '❌ لا يمكن بدء حرب بين نفس الكلان!' };
  }
  
  // التحقق من عدم وجود حرب جارية
  if (clan1.wars?.currentWar) {
    return { success: false, message: '❅ الكلان الأول في حرب بالفعل!' };
  }
  
  if (clan2.wars?.currentWar) {
    return { success: false, message: '❅ الكلان الثاني في حرب بالفعل!' };
  }
  
  // حساب الجائزة
  const prizePool = Math.floor(((clan1.gold || 0) * 0.1 + (clan2.gold || 0) * 0.1)) + 1000;
  ...
  const war = {
    ...
    clan2: {
      ...
      totalDamage: 0,
      destruction: 0,
      participants: [...clan2.members]
    },
    startedAt: Date.now(),
    preparationEnds: Date.now() + 15 * 60 * 1000, // 15 دقيقة تحضير
    warEnds: Date.now() + 30 * 60 * 1000, // 30 دقيقة حرب
    status: 'preparation',
    prizePool: prizePool,
    reportsSent: 0
  };
  ...
  // تحديث الحالة
  clan1.wars = clan1.wars || {};
  clan1.wars.currentWar = war;
  
  clan2.wars = clan2.wars || {};
  clan2.wars.currentWar = war;
  
  saveDatabase();
  
  // إرسال إشعار للقناة
  try {
    const channelMessage = `🔔 *تم قبول التحدي!*\n\n⚔️ ستبدأ المعركة خلال 15 دقيقة\n\n🏰 *الكلانات المتحاربة:*\n• ${clan1.name} (مستوى ${clan1.level})\n• ${clan2.name} (مستوى ${clan2.level})\n\n💰 الجائزة: ${prizePool.toLocaleString()} ذهب`;
    
    if (sock && WAR_CHANNEL) {
      await sock.sendMessage(WAR_CHANNEL, { text: channelMessage });
    }
  } catch (e) {
    console.error('❌ خطأ في إرسال إشعار القبول:', e.message);
  }
  
  // جدولة بدء الحرب بعد 15 دقيقة
  setTimeout(async () => {
    await onWarStarted(war, clan1, clan2, sock);
  }, 15 * 60 * 1000);
  
  return {
    success: true,
    war,
    message: `⚔️ تم بدء الحرب!\n🏰 ${clan1.name} vs ${clan2.name}\n⏰ 15 دقيقة تحضير\n💰 الجائزة: ${prizePool.toLocaleString()}`
  };
};

// دالة تُستدعى عند بدء الحرب فعلياً
export const onWarStarted = async (war, clan1, clan2, sock) => {
  try {
    // تحديث حالة الحرب
    war.status = 'active';
    ...
    const rpgData = getRpgData();
    const clan1Members = clan1.members.map(mid => {
      const player = rpgData.players[mid];
      return player ? player.name : 'غير معروف';
    }).slice(0, 10).join('\n• ');
    
    const clan2Members = clan2.members.map(mid => {
      const player = rpgData.players[mid];
      return player ? player.name : 'غير معروف';
    }).slice(0, 10).join('\n• ');
    
    // رسالة بدء الحرب
    const warStartMessage = `⚔️ *انطلقت الحرب!* ⚔️\n\n🏰 *الكلانات المتحاربة:*\n• ${clan1.name} (مستوى ${clan1.level})\n• ${clan2.name} (مستوى ${clan2.level})\n\n👥 *المشاركين:*\n\n${clan1.name}:\n• ${clan1Members}\n\n${clan2.name}:\n• ${clan2Members}\n\n⏱️ مدة الحرب: 30 دقيقة\n💰 الجائزة: ${(war.prizePool || 1000).toLocaleString()} ذهب\n\n🔥 فليبدأ القتال!`;
    
    if (sock && WAR_CHANNEL) {
      await sock.sendMessage(WAR_CHANNEL, { text: warStartMessage });
    }
    
    // بدء التقارير اللحظية كل 5 دقائق
    war.reportInterval = setInterval(() => {
      sendWarReport(war, clan1, clan2, sock);
    }, 5 * 60 * 1000);
    
    saveDatabase();
  } catch (e) {
    console.error('❌ خطأ في بدء الحرب:', e.message);
  }
};

// إرسال تقرير لحظي عن سير المعركة
export const sendWarReport = async (war, clan1, clan2, sock) => {
  try {
    war.reportsSent = (war.reportsSent || 0) + 1;
    
    const timeElapsed = Math.floor((Date.now() - war.startedAt) / 60000);
    const timeRemaining = 30 - timeElapsed;
    
    if (timeRemaining <= 0) {
      clearInterval(war.reportInterval);
      return;
    }
    
    const clan1Destruction = war.clan1.destruction || 0;
    const clan2Destruction = war.clan2.destruction || 0;
    
    const leader = clan1Destruction > clan2Destruction ? war.clan1.name : (clan2Destruction > clan1Destruction ? war.clan2.name : 'متكافئ');
    
    const reportMessage = `📊 *تقرير المعركة #${war.reportsSent}*\n\n⚔️ سير القتال:\n• ${war.clan1.name} دمر ${clan1Destruction.toFixed(1)}% من ${war.clan2.name}\n• ${war.clan2.name} دمر ${clan2Destruction.toFixed(1)}% من ${war.clan1.name}\n\n🏆 المتصدر: ${leader}\n⏱️ الوقت المتبقي: ${timeRemaining} دقيقة\n\n💀 استمروا في القتال!`;
    
    if (sock && WAR_CHANNEL) {
      await sock.sendMessage(WAR_CHANNEL, { text: reportMessage });
    }
    
    saveDatabase();
  } catch (e) {
    console.error('❌ خطأ في إرسال التقرير:', e.message);
  }
};
