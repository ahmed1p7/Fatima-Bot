// ═══════════════════════════════════════════════════════════════════════════════
// 📜 نظام المهام والإنجازات - فاطمة بوت v11.0
// يتضمن: مهام يومية، أسبوعية، شهرية، إنجازات
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════════
// 📋 الحصول على مهام اللاعب
// ═══════════════════════════════════════════════════════════════════════════════

export const getPlayerQuests = (player) => {
  const now = Date.now();
  ...
  if (now > player.quests.lastDailyReset + 24 * 60 * 60 * 1000) {
    player.quests.daily = generateDailyQuests(player.level);
    player.quests.lastDailyReset = now;
  }
  
  // التحقق من إعادة التعيين الأسبوعي
  if (now > player.quests.lastWeeklyReset + 7 * 24 * 60 * 60 * 1000) {
    player.quests.weekly = generateWeeklyQuests(player.level);
    player.quests.lastWeeklyReset = now;
  }
  
  // إزالة المهام المنتهية
  player.quests.daily = player.quests.daily.filter(q => q.expiresAt > now && !q.claimed);
  player.quests.weekly = player.quests.weekly.filter(q => q.expiresAt > now && !q.claimed);
...
export const claimQuestReward = (player, questId) => {
  ...
  // البحث في المهام اليومية
  let quest = quests.daily.find(q => q.id === questId);
  
  // البحث في المهام الأسبوعية إذا لم توجد في اليومية
  if (!quest) {
...
export const formatQuests = (player) => {
  ...
  if (quests.daily.length === 0) {
    ...
    for (const quest of quests.daily) {
      ...
      const progressBar = '▓'.repeat(Math.floor((quest.progress / quest.target) * 5)) + 
                          '░'.repeat(5 - Math.floor((quest.progress / quest.target) * 5));
      ...
      if (quest.completed && !quest.claimed) {
        text += `   🎁 استخدم .مطالبة ${quest.id.slice(0, 8)}\n`;
      }
      text += `\n`;
    }
  }
  ...
  // الوقت المتبقي
  const now = Date.now();
  const dailyRemaining = quests.lastDailyReset + 24 * 60 * 60 * 1000 - now;
  const weeklyRemaining = quests.lastWeeklyReset + 7 * 24 * 60 * 60 * 1000 - now;
...
export const resetExpiredQuests = (player) => {
  ...
  // إزالة المهام اليومية المنتهية
  player.quests.daily = player.quests.daily.filter(q => q.expiresAt > now);
  
  // إزالة المهام الأسبوعية المنتهية
  player.quests.weekly = player.quests.weekly.filter(q => q.expiresAt > now);
