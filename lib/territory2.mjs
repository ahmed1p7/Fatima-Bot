// ═══════════════════════════════════════════════════════════════════════════════
// 🗺️ نظام الأقاليم الموحد مع الصور - فاطمة بوت v12.0
// يجمع نظام الأقاليم مع دوال إرسال الصور
// ═══════════════════════════════════════════════════════════════════════════════

import { getRpgData, saveDatabase } from './database.mjs';
import { healthBar, xpForLevel } from './rpg.mjs';

// ═══════════════════════════════════════════════════════════════════════════════
// 🔗 روابط الصور الافتراضية
// ═══════════════════════════════════════════════════════════════════════════════

export const DEFAULT_IMAGE_URL = 'https://files.catbox.moe/p4mtw3.jpg';

export const IMAGE_URLS = {
  classes: {
    'محارب': DEFAULT_IMAGE_URL,
    'ساحر': DEFAULT_IMAGE_URL,
    'رامي': DEFAULT_IMAGE_URL,
    'شافي': DEFAULT_IMAGE_URL,
    'قاتل': DEFAULT_IMAGE_URL,
    'فارس': DEFAULT_IMAGE_URL,
    'default': DEFAULT_IMAGE_URL
  },
  bosses: {
    'kaska': DEFAULT_IMAGE_URL,
    'orc_king': DEFAULT_IMAGE_URL,
    'ant_king': DEFAULT_IMAGE_URL,
    'igrys': DEFAULT_IMAGE_URL,
    'dragon': DEFAULT_IMAGE_URL,
    'swamp_king': DEFAULT_IMAGE_URL,
    'default': DEFAULT_IMAGE_URL
  },
  profile: {
    default: DEFAULT_IMAGE_URL,
    warrior: DEFAULT_IMAGE_URL,
    mage: DEFAULT_IMAGE_URL
  },
  clan: {
    default: DEFAULT_IMAGE_URL,
    war: DEFAULT_IMAGE_URL
  },
  territory: {
    gold_mine: DEFAULT_IMAGE_URL,
    crystal_cave: DEFAULT_IMAGE_URL,
    dark_forest: DEFAULT_IMAGE_URL,
    dragon_peak: DEFAULT_IMAGE_URL,
    ancient_ruins: DEFAULT_IMAGE_URL,
    default: DEFAULT_IMAGE_URL
  },
  boxes: {
    'خشبي': DEFAULT_IMAGE_URL,
    'حديدي': DEFAULT_IMAGE_URL,
    'ذهبي': DEFAULT_IMAGE_URL
  },
  general: {
    logo: DEFAULT_IMAGE_URL,
    banner: DEFAULT_IMAGE_URL
  }
};

// ═══════════════════════════════════════════════════════════════════════════════
// 📤 دالة إرسال الصورة (مدمجة)
// ═══════════════════════════════════════════════════════════════════════════════

export const sendImage = async (sock, jid, options) => {
  const { url, buffer, caption = '', quoted = null, contextInfo = null } = options;
  try {
    if (url) {
      const message = { image: { url }, caption, mimetype: 'image/jpeg', ...(contextInfo && { contextInfo }) };
      return await sock.sendMessage(jid, message, { quoted });
    }
    if (buffer) {
      const message = { image: buffer, caption, mimetype: 'image/jpeg', ...(contextInfo && { contextInfo }) };
      return await sock.sendMessage(jid, message, { quoted });
    }
    throw new Error('يجب توفير url أو buffer');
  } catch (error) {
    console.error('❌ خطأ في إرسال الصورة:', error.message);
    if (caption) return await sock.sendMessage(jid, { text: caption }, { quoted });
    return null;
  }
};

// ═══════════════════════════════════════════════════════════════════════════════
// 🖼️ دوال إرسال الصور المتخصصة
// ═══════════════════════════════════════════════════════════════════════════════

export const sendProfileImage = async (sock, jid, player, options = {}) => {
  const { RPG } = await import('./rpg.mjs');
  const classEmoji = RPG.classes[player.class]?.emoji || '🎮';
  const classData = RPG.classes[player.class];
  const imageUrl = IMAGE_URLS.classes[player.class] || IMAGE_URLS.profile.default;
  const caption = formatPlayerCaption(player, classData, classEmoji);
  const contextInfo = {
    externalAdReply: {
      title: `${classEmoji} ${player.name} - Lv.${player.level}`,
      body: `${player.class} | ⚔️${player.atk} 🛡️${player.def}`,
      thumbnailUrl: imageUrl,
      sourceUrl: 'https://whatsapp.com/channel/0029VbCbgwIKgsNxh9vKb01n',
      mediaType: 1,
      renderLargerThumbnail: true
    }
  };
  return await sendImage(sock, jid, { url: imageUrl, caption, contextInfo, quoted: options.quoted });
};

export const sendBossImage = async (sock, jid, boss, options = {}) => {
  const imageUrl = IMAGE_URLS.bosses[boss.id] || IMAGE_URLS.bosses.default;
  const caption = `@
━─━••❁⊰｢❀｣⊱❁••━─━

👹 • • ✤ ${boss.name} ✤ • • 👹

┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
│ 🏷️ النوع: ${boss.type}
│ ⭐ المستوى: ${boss.level}
│ ❤️ HP: ${boss.hp?.toLocaleString() || '???'}
│ ⚔️ ATK: ${boss.atk || '???'}
│ 🛡️ DEF: ${boss.def || '???'}
┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈

${options.additionalText || ''}

> \`بــوت :\`
> _*『 FATIMA 』*_
━─━••❁⊰｢ ❀｣⊱❁••━─━`;
  const contextInfo = {
    externalAdReply: {
      title: `${boss.emoji} ${boss.name}`,
      body: `زعيم ${boss.type} - المستوى ${boss.level}`,
      thumbnailUrl: imageUrl,
      sourceUrl: 'https://whatsapp.com/channel/0029VbCbgwIKgsNxh9vKb01n',
      mediaType: 1,
      renderLargerThumbnail: true
    }
  };
  return await sendImage(sock, jid, { url: imageUrl, caption, contextInfo, quoted: options.quoted });
};

export const sendClanImage = async (sock, jid, clan, options = {}) => {
  const imageUrl = options.isWar ? IMAGE_URLS.clan.war : IMAGE_URLS.clan.default;
  const caption = `@
━─━••❁⊰｢❀｣⊱❁••━─━

🏰 • • ✤ ${clan.name} ✤ • • 🏰

┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
│ 🏷️ Tag: #${clan.clanTag}
│ ⭐ المستوى: ${clan.level || 1}
│ 👥 الأعضاء: ${clan.members?.length || 1}
│ 🏆 الانتصارات: ${clan.wins || 0}
│ ❌ الخسائر: ${clan.losses || 0}
┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈

💰 الموارد:
│ الذهب: ${(clan.resources?.gold || clan.gold || 0).toLocaleString()}
│ الإكسير: ${(clan.resources?.elixir || 0).toLocaleString()}

${options.additionalText || ''}

> \`بــوت :\`
> _*『 FATIMA 』*_
━─━••❁⊰｢ ❀｣⊱❁••━─━`;
  const contextInfo = {
    externalAdReply: {
      title: `🏰 ${clan.name} #${clan.clanTag}`,
      body: `Lv.${clan.level || 1} | ${clan.members?.length || 1} عضو`,
      thumbnailUrl: imageUrl,
      sourceUrl: 'https://whatsapp.com/channel/0029VbCbgwIKgsNxh9vKb01n',
      mediaType: 1
    }
  };
  return await sendImage(sock, jid, { url: imageUrl, caption, contextInfo, quoted: options.quoted });
};

export const sendTerritoryImage = async (sock, jid, territory, options = {}) => {
  const imageUrl = IMAGE_URLS.territory[territory.type] || IMAGE_URLS.territory.default;
  const ownerText = territory.ownerClan ? `🏰 المالك: ${options.clanName || 'كلان غير معروف'}` : '🏳️ غير محتل';
  const guardianText = territory.guardian ? `\n👹 الحارس: ${territory.guardian.name} (Lv.${territory.guardian.level})` : '';
  const caption = `@
━─━••❁⊰｢❀｣⊱❁••━─━

🗺️ • • ✤ ${territory.name} ✤ • • 🗺️

┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
│ 📍 النوع: ${territory.typeName || territory.type}
│ ${ownerText}${guardianText}
┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈

📦 الإنتاج اليومي:
│ 💰 ${(territory.production?.gold || 0).toLocaleString()} ذهب
│ ⚗️ ${(territory.production?.elixir || 0).toLocaleString()} إكسير
│ 🪵 ${(territory.production?.wood || 0).toLocaleString()} خشب

🛡️ الحامية: ${territory.garrison?.total || 0} جندي

${options.additionalText || ''}

> \`بــوت :\`
> _*『 FATIMA 』*_
━─━••❁⊰｢ ❀｣⊱❁••━─━`;
  const contextInfo = {
    externalAdReply: {
      title: `🗺️ ${territory.name}`,
      body: territory.ownerClan ? `محتول من كلان` : 'متاح للاحتلال',
      thumbnailUrl: imageUrl,
      sourceUrl: 'https://whatsapp.com/channel/0029VbCbgwIKgsNxh9vKb01n',
      mediaType: 1
    }
  };
  return await sendImage(sock, jid, { url: imageUrl, caption, contextInfo, quoted: options.quoted });
};

export const sendBoxImage = async (sock, jid, boxType, contents, options = {}) => {
  const imageUrl = IMAGE_URLS.boxes[boxType] || IMAGE_URLS.general.banner;
  let contentsText = '📋 المحتويات:\n';
  if (contents.weapon) contentsText += `⚔️ ${contents.weapon.fullName}\n   ATK: +${contents.weapon.atk}\n`;
  if (contents.armor) contentsText += `🛡️ ${contents.armor.fullName}\n   DEF: +${contents.armor.def}\n`;
  if (contents.gold) contentsText += `💰 ${contents.gold} ذهب\n`;
  if (contents.xp) contentsText += `⭐ ${contents.xp} XP\n`;
  const caption = `@
━─━••❁⊰｢❀｣⊱❁••━─━

📦 • • ✤ صندوق ${boxType} ✤ • • 📦

┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
${contentsText}
┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈

> \`بــوت :\`
> _*『 FATIMA 』*_
━─━••❁⊰｢ ❀｣⊱❁••━─━`;
  return await sendImage(sock, jid, { url: imageUrl, caption, quoted: options.quoted });
};

export const sendWithThumbnail = async (sock, jid, text, imageUrl, title, body) => {
  return await sock.sendMessage(jid, {
    text,
    contextInfo: {
      externalAdReply: {
        title, body,
        thumbnailUrl: imageUrl,
        sourceUrl: 'https://whatsapp.com/channel/0029VbCbgwIKgsNxh9vKb01n',
        mediaType: 1,
        renderLargerThumbnail: true
      }
    }
  });
};

// ═══════════════════════════════════════════════════════════════════════════════
// 🔧 دوال مساعدة لتنسيق النص
// ═══════════════════════════════════════════════════════════════════════════════

const formatPlayerCaption = (player, classData, classEmoji) => {
  const hpBarText = healthBar(player.hp, player.maxHp);
  const xpBarText = healthBar(player.xp, xpForLevel(player.level), 10);
  return `@
━─━••❁⊰｢❀｣⊱❁••━─━

${classEmoji} • • ✤ ${player.name} ✤ • • ${classEmoji}

┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈
│ 🎖️ المستوى: ${player.level}
│ ⭐ XP: [${xpBarText}] ${player.xp}/${xpForLevel(player.level)}
┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈

❤️ الصحة: ${player.hp}/${player.maxHp}
│ [${hpBarText}]

⚡ الطاقة: ${player.stamina}/${player.maxStamina || 10}

┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈

📊 الإحصائيات:
│ ⚔️ هجوم: ${player.atk}
│ 🛡️ دفاع: ${player.def}
│ ✨ سحر: ${player.mag}

💰 الذهب: ${(player.gold || 0).toLocaleString()}
🏆 الانتصارات: ${player.wins || 0}

┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈

> \`بــوت :\`
> _*『 FATIMA 』*_
━─━••❁⊰｢ ❀｣⊱❁••━─━`;
};

// ═══════════════════════════════════════════════════════════════════════════════
// 🗺️ تعريف الأقاليم الأساسية
// ═══════════════════════════════════════════════════════════════════════════════

export const TERRITORY_TYPES = {
  gold_mine: {
    id: 'gold_mine',
    name: 'منجم الذهب',
    emoji: '⛏️',
    typeName: 'إنتاج ذهب',
    production: { gold: 500 },
    guardianLevel: [10, 20, 30, 40, 50],
    guardianNames: ['غول المنجم', 'عفريت الذهب', 'تنين الكهف', 'حارس الكنز', 'ملك المناجم']
  },
  crystal_cave: {
    id: 'crystal_cave',
    name: 'كهف الكريستال',
    emoji: '💎',
    typeName: 'إنتاج إكسير',
    production: { elixir: 300 },
    guardianLevel: [15, 25, 35, 45, 55],
    guardianNames: ['عنكبوت كريستالي', 'غول البلور', 'حارس الأحجار', 'ساحر الكهف', 'ملك الكريستال']
  },
  dark_forest: {
    id: 'dark_forest',
    name: 'الغابة المظلمة',
    emoji: '🌲',
    typeName: 'إنتاج خشب',
    production: { wood: 400 },
    guardianLevel: [8, 18, 28, 38, 48],
    guardianNames: ['ذئب ضخم', 'دب الغابة', 'شجرة حية', 'ساحر الغابة', 'روح الغابة']
  },
  dragon_peak: {
    id: 'dragon_peak',
    name: 'قمة التنين',
    emoji: '🏔️',
    typeName: 'إنتاج متعدد',
    production: { gold: 300, elixir: 200, wood: 100 },
    guardianLevel: [30, 40, 50, 60, 70],
    guardianNames: ['تنين صغير', 'تنين أزرق', 'تنين أحمر', 'تنين أسود', 'ملك التنانين']
  },
  ancient_ruins: {
    id: 'ancient_ruins',
    name: 'الأطلال القديمة',
    emoji: '🏛️',
    typeName: 'إنتاج نادر',
    production: { gold: 200, elixir: 400 },
    guardianLevel: [25, 35, 45, 55, 65],
    guardianNames: ['محارب قديم', 'فرعون ملعون', 'مومياء', 'حارس المقبرة', 'ملك الأطلال']
  },
  swamp_lands: {
    id: 'swamp_lands',
    name: 'أراضي المستنقعات',
    emoji: '🐸',
    typeName: 'إنتاج حجري',
    production: { stone: 350 },
    guardianLevel: [12, 22, 32, 42, 52],
    guardianNames: ['ضفدع عملاق', 'تمساح المستنقع', 'وحل حي', 'ساحرة المستنقع', 'ملك المستنقعات']
  }
};

// ═══════════════════════════════════════════════════════════════════════════════
// 🆕 إنشاء إقليم جديد
// ═══════════════════════════════════════════════════════════════════════════════

export const createTerritory = (typeId, name = null) => {
  const type = TERRITORY_TYPES[typeId];
  if (!type) return null;
  const territoryId = `territory_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`;
  const guardianTier = Math.floor(Math.random() * type.guardianLevel.length);
  return {
    id: territoryId,
    type: typeId,
    name: name || type.name,
    emoji: type.emoji,
    typeName: type.typeName,
    production: { ...type.production },
    guardian: {
      name: type.guardianNames[guardianTier],
      level: type.guardianLevel[guardianTier],
      tier: guardianTier,
      hp: type.guardianLevel[guardianTier] * 100,
      atk: Math.floor(type.guardianLevel[guardianTier] * 4),
      def: Math.floor(type.guardianLevel[guardianTier] * 2)
    },
    ownerClan: null,
    capturedAt: null,
    garrison: { soldiers: [], total: 0, defense: 0 },
    taxRate: 0.10,
    totalCollected: { gold: 0, elixir: 0, wood: 0, stone: 0 },
    created: Date.now(),
    lastProduction: Date.now()
  };
};

// ═══════════════════════════════════════════════════════════════════════════════
// 🔧 دوال مساعدة للاعبين
// ═══════════════════════════════════════════════════════════════════════════════

const updatePlayerStamina = (player) => {
  const now = Date.now();
  const maxStamina = 100 + (player.level || 1) * 5;
  const lastUpdate = player.lastStaminaUpdate || now;
  const elapsed = now - lastUpdate;
  const regen = Math.floor(elapsed / 60000);
  if (regen > 0) {
    player.stamina = Math.min(maxStamina, (player.stamina || 0) + regen);
    player.lastStaminaUpdate = now;
  }
  return player.stamina;
};

const getClanById = (clanId) => {
  const data = getRpgData();
  return data.clans?.[clanId] || null;
};

// ═══════════════════════════════════════════════════════════════════════════════
// 🗺️ استكشاف إقليم
// ═══════════════════════════════════════════════════════════════════════════════

export const exploreTerritory = (player) => {
  const rpgData = getRpgData();
  const stamina = updatePlayerStamina(player);
  if (stamina < 1) return { success: false, message: '❌ لا تملك طاقة كافية!' };
  player.stamina = stamina - 1;
  const types = Object.keys(TERRITORY_TYPES);
  const randomType = types[Math.floor(Math.random() * types.length)];
  const territory = createTerritory(randomType);
  if (!rpgData.territories) rpgData.territories = {};
  rpgData.territories[territory.id] = territory;
  saveDatabase();
  return { success: true, territory, message: `🗺️ اكتشفت ${territory.emoji} ${territory.name}!\n👹 يحرسه: ${territory.guardian.name} (Lv.${territory.guardian.level})` };
};

// ═══════════════════════════════════════════════════════════════════════════════
// ⚔️ قتال الحارس
// ═══════════════════════════════════════════════════════════════════════════════

export const fightGuardian = (player, territoryId, clanId) => {
  const rpgData = getRpgData();
  const territory = rpgData.territories?.[territoryId];
  if (!territory) return { success: false, message: '❌ الإقليم غير موجود!' };
  if (!clanId) return { success: false, message: '❌ يجب أن تكون في كلان للاحتلال!' };
  if (territory.ownerClan) return { success: false, message: '❌ هذا الإقليم محتل بالفعل!' };
  const stamina = updatePlayerStamina(player);
  if (stamina < 1) return { success: false, message: '❌ لا تملك طاقة كافية!' };
  if (player.hp <= player.maxHp * 0.2) return { success: false, message: '❌ صحتك ضعيفة! استخدم .علاج أولاً' };
  player.stamina = stamina - 1;
  const guardian = territory.guardian;
  if (!guardian) return { success: false, message: '❌ لا يوجد حارس في هذا الإقليم!' };
  let playerHp = player.hp, guardianHp = guardian.hp;
  while (playerHp > 0 && guardianHp > 0) {
    const playerDmg = Math.max(1, (player.atk || 10) - guardian.def + Math.floor(Math.random() * 20));
    guardianHp -= playerDmg;
    if (guardianHp <= 0) break;
    const guardianDmg = Math.max(1, guardian.atk - (player.def || 0) + Math.floor(Math.random() * 15));
    playerHp -= guardianDmg;
  }
  if (playerHp > 0) {
    player.hp = Math.min(player.maxHp, playerHp);
    player.xp = (player.xp || 0) + guardian.level * 10;
    player.gold = (player.gold || 0) + guardian.level * 50;
    territory.ownerClan = clanId;
    territory.capturedAt = Date.now();
    territory.guardian = null;
    territory.lastProduction = Date.now();
    saveDatabase();
    return { success: true, won: true, message: `🏆 انتصرت على ${guardian.name}!\n🏰 تم احتلال ${territory.name} لصالح كلانك!`, rewards: { xp: guardian.level * 10, gold: guardian.level * 50 } };
  } else {
    player.hp = Math.floor(player.maxHp * 0.3);
    player.losses = (player.losses || 0) + 1;
    saveDatabase();
    return { success: true, won: false, message: `💔 هُزمت بواسطة ${guardian.name}!` };
  }
};

// ═══════════════════════════════════════════════════════════════════════════════
// 🕵️ التسلل (للقاتل والرامي فقط)
// ═══════════════════════════════════════════════════════════════════════════════

export const stealthRaid = (player, territoryId) => {
  const rpgData = getRpgData();
  const territory = rpgData.territories?.[territoryId];
  if (!territory) return { success: false, message: '❌ الإقليم غير موجود!' };
  if (!['قاتل', 'رامي'].includes(player.class)) return { success: false, message: '❌ التسلل متاح فقط للقتلة والرماة!' };
  if (!territory.ownerClan) return { success: false, message: '❌ الإقليم غير محتل، لا يمكن التسلل!' };
  const stamina = updatePlayerStamina(player);
  if (stamina < 1) return { success: false, message: '❌ لا تملك طاقة كافية!' };
  player.stamina = stamina - 1;
  const success = Math.random() < 0.5;
  if (success) {
    const stolen = {};
    for (const [resource, amount] of Object.entries(territory.production)) {
      stolen[resource] = Math.floor(amount * 0.5);
      if (resource === 'gold') player.gold = (player.gold || 0) + stolen[resource];
      if (resource === 'elixir') player.elixir = (player.elixir || 0) + stolen[resource];
    }
    saveDatabase();
    return { success: true, stolen, message: `🕵️ نجح التسلل!\n💰 سرقت:\n${Object.entries(stolen).map(([r, a]) => `${r}: ${a}`).join('\n')}` };
  } else {
    const damage = Math.floor(territory.guardian?.atk || 20);
    player.hp = Math.max(1, player.hp - damage);
    saveDatabase();
    return { success: true, failed: true, message: `🚨 اكتشفك الحارس!\n💔 خسرت ${damage} HP` };
  }
};

// ═══════════════════════════════════════════════════════════════════════════════
// 🛡️ نشر جنود في الحامية
// ═══════════════════════════════════════════════════════════════════════════════

export const deployGarrison = (player, territoryId, soldierCount) => {
  const rpgData = getRpgData();
  const territory = rpgData.territories?.[territoryId];
  if (!territory) return { success: false, message: '❌ الإقليم غير موجود!' };
  if (territory.ownerClan !== player.clanId) return { success: false, message: '❌ هذا الإقليم لا يتبع لكلانك!' };
  if (!player.soldiers || player.soldiers < soldierCount) return { success: false, message: `❌ ليس لديك ${soldierCount} جندي! (لديك ${player.soldiers || 0})` };
  territory.garrison.soldiers.push({
    playerId: player.id,
    playerName: player.name,
    playerClass: player.class,
    count: soldierCount,
    deployedAt: Date.now()
  });
  territory.garrison.total += soldierCount;
  territory.garrison.defense += soldierCount * (player.def || 10);
  player.soldiers -= soldierCount;
  saveDatabase();
  return { success: true, message: `🛡️ تم نشر ${soldierCount} جندي في ${territory.name}!\n📊 إجمالي الحامية: ${territory.garrison.total} جندي` };
};

// ═══════════════════════════════════════════════════════════════════════════════
// ⚔️ غزو إقليم
// ═══════════════════════════════════════════════════════════════════════════════

export const invadeTerritory = (attackers, territoryId, attackerClanId) => {
  const rpgData = getRpgData();
  const territory = rpgData.territories?.[territoryId];
  if (!territory) return { success: false, message: '❌ الإقليم غير موجود!' };
  if (territory.ownerClan === attackerClanId) return { success: false, message: '❌ لا يمكنك غزو إقليم كلانك!' };
  if (!territory.ownerClan) return { success: false, message: '❌ هذا الإقليم غير محتل! استخدم .قتال_حارس أولاً' };
  let totalAttack = 0;
  for (const attacker of attackers) totalAttack += (attacker.atk || 10) + Math.floor(Math.random() * 10);
  const totalDefense = territory.garrison?.defense || 0;
  const attackerPower = totalAttack * (1 + Math.random() * 0.3);
  const defenderPower = totalDefense * (1 + Math.random() * 0.3);
  const attackerLosses = Math.floor(defenderPower * 0.1);
  const defenderLosses = Math.floor(attackerPower * 0.1);
  if (attackerPower > defenderPower) {
    const previousOwner = territory.ownerClan;
    territory.ownerClan = attackerClanId;
    territory.capturedAt = Date.now();
    territory.garrison = { soldiers: [], total: 0, defense: 0 };
    const looted = {};
    for (const [resource, amount] of Object.entries(territory.production)) {
      looted[resource] = Math.floor(amount * 2);
      if (resource === 'gold') {
        for (const attacker of attackers) attacker.gold = (attacker.gold || 0) + looted[resource] / attackers.length;
      }
    }
    for (const attacker of attackers) attacker.xp = (attacker.xp || 0) + 50;
    const defenderClan = getClanById(previousOwner);
    if (defenderClan) defenderClan.losses = (defenderClan.losses || 0) + 1;
    const attackerClan = getClanById(attackerClanId);
    if (attackerClan) attackerClan.wins = (attackerClan.wins || 0) + 1;
    saveDatabase();
    return { success: true, won: true, message: `🏆 انتصار!\n🏰 تم احتلال ${territory.name}!\n💰 الغنائم:\n${Object.entries(looted).map(([r, a]) => `${r}: ${a}`).join('\n')}` };
  } else {
    for (const attacker of attackers) attacker.hp = Math.max(1, attacker.hp - Math.floor(attackerLosses / attackers.length));
    if (territory.garrison) {
      territory.garrison.total = Math.max(0, territory.garrison.total - defenderLosses);
      territory.garrison.defense = Math.max(0, territory.garrison.defense - defenderLosses * 10);
    }
    saveDatabase();
    return { success: true, won: false, message: `🛡️ صمدت الدفاعات!\n💔 خسرتم المعركة` };
  }
};

// ═══════════════════════════════════════════════════════════════════════════════
// 📋 عرض معلومات الإقليم
// ═══════════════════════════════════════════════════════════════════════════════

export const formatTerritoryInfo = (territory, clanName = null) => {
  let text = `🗺️ ═══════ ${territory.name} ═══════ 🗺️\n\n`;
  text += `${territory.emoji} النوع: ${territory.typeName}\n`;
  if (territory.ownerClan) text += `🏰 المالك: ${clanName || 'كلان غير معروف'}\n`;
  else text += `🏳️ غير محتل\n`;
  text += `\n📦 الإنتاج اليومي:\n`;
  for (const [resource, amount] of Object.entries(territory.production || {})) text += `   ${resource}: ${amount}\n`;
  if (territory.guardian) text += `\n👹 الحارس:\n   ${territory.guardian.name} (Lv.${territory.guardian.level})\n   ❤️ HP: ${territory.guardian.hp}\n`;
  if (territory.garrison?.total > 0) text += `\n🛡️ الحامية:\n   إجمالي الجنود: ${territory.garrison.total}\n   قوة الدفاع: ${territory.garrison.defense}\n`;
  return text;
};

// ═══════════════════════════════════════════════════════════════════════════════
// 📋 عرض الحامية
// ═══════════════════════════════════════════════════════════════════════════════

export const formatGarrisonInfo = (territory) => {
  if (!territory.garrison || territory.garrison.soldiers.length === 0) return '🛡️ لا توجد حامية في هذا الإقليم!';
  let text = `🛡️ ═══════ حامية ${territory.name} ═══════ 🛡️\n\n`;
  for (const soldier of territory.garrison.soldiers) {
    const emoji = { 'محارب': '⚔️', 'فارس': '🛡️', 'رامي': '🏹', 'ساحر': '🧙', 'شافي': '💚', 'قاتل': '🗡️' };
    text += `${emoji[soldier.playerClass] || '👤'} ${soldier.playerName}\n   👥 ${soldier.count} جندي\n\n`;
  }
  text += `📊 الإجمالي: ${territory.garrison.total} جندي\n🛡️ قوة الدفاع: ${territory.garrison.defense}`;
  return text;
};

// ═══════════════════════════════════════════════════════════════════════════════
// 🔄 سحب الجنود
// ═══════════════════════════════════════════════════════════════════════════════

export const withdrawGarrison = (player, territoryId) => {
  const rpgData = getRpgData();
  const territory = rpgData.territories?.[territoryId];
  if (!territory) return { success: false, message: '❌ الإقليم غير موجود!' };
  const index = territory.garrison?.soldiers?.findIndex(s => s.playerId === player.id);
  if (index === -1 || index === undefined) return { success: false, message: '❌ ليس لديك جنود في هذا الإقليم!' };
  const soldier = territory.garrison.soldiers[index];
  player.soldiers = (player.soldiers || 0) + soldier.count;
  territory.garrison.soldiers.splice(index, 1);
  territory.garrison.total -= soldier.count;
  territory.garrison.defense -= soldier.count * (player.def || 10);
  saveDatabase();
  return { success: true, count: soldier.count, message: `✅ تم سحب ${soldier.count} جندي من الحامية!` };
};

// ═══════════════════════════════════════════════════════════════════════════════
// 📋 قائمة أقاليم الكلان
// ═══════════════════════════════════════════════════════════════════════════════

export const getClanTerritories = (clanId) => {
  const rpgData = getRpgData();
  const territories = [];
  for (const [id, territory] of Object.entries(rpgData.territories || {})) {
    if (territory.ownerClan === clanId) territories.push(territory);
  }
  return territories;
};

// ═══════════════════════════════════════════════════════════════════════════════
// 💰 جمع الإنتاج اليومي
// ═══════════════════════════════════════════════════════════════════════════════

export const collectTerritoryProduction = (clanId) => {
  const rpgData = getRpgData();
  const now = Date.now();
  const collected = { gold: 0, elixir: 0, wood: 0, stone: 0 };
  for (const territory of Object.values(rpgData.territories || {})) {
    if (territory.ownerClan !== clanId) continue;
    const hours = Math.floor((now - territory.lastProduction) / 3600000);
    if (hours <= 0) continue;
    for (const [resource, amount] of Object.entries(territory.production)) {
      const total = amount * hours;
      if (resource === 'gold') collected.gold += total;
      if (resource === 'elixir') collected.elixir += total;
      if (resource === 'wood') collected.wood += total;
      if (resource === 'stone') collected.stone += total;
    }
    territory.lastProduction = now;
  }
  const clan = getClanById(clanId);
  if (clan) {
    clan.resources = clan.resources || {};
    for (const [res, amount] of Object.entries(collected)) if (amount > 0) clan.resources[res] = (clan.resources[res] || 0) + amount;
    saveDatabase();
  }
  return collected;
};

export default {
  DEFAULT_IMAGE_URL, IMAGE_URLS,
  sendImage, sendProfileImage, sendBossImage, sendClanImage, sendTerritoryImage, sendBoxImage, sendWithThumbnail,
  TERRITORY_TYPES, createTerritory, exploreTerritory, fightGuardian, stealthRaid,
  deployGarrison, invadeTerritory, formatTerritoryInfo, formatGarrisonInfo,
  withdrawGarrison, getClanTerritories, collectTerritoryProduction
};
