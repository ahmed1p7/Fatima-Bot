...
export const generateResponse = async (message, groupId, senderId, senderName) => {
  ...
  const isCommand = msgBody.startsWith('.');
  
  // عدم الرد على الأوامر مباشرة
  if (isCommand) return null;

  // فرصة عشوائية للرد (5% فقط حتى لا تكون مزعجة)
  const randomChance = Math.random();
  if (randomChance > 0.05) return null;
  ...
  if (lowerMsg.includes('مرحبا') || lowerMsg.includes('هلا') || lowerMsg.includes('سلام')) {
    prompt = `رد بسيط وودي على التحية باللهجة العربية العامية، اسم المرسل: ${senderName}`;
  } else if (lowerMsg.includes('شكرا') || lowerMsg.includes('thanks')) {
  ...
  } else if (lowerMsg.includes('كيف') || lowerMsg.includes('شنو') || lowerMsg.includes('واش')) {
    // سؤال عام - استخدم AI
    try {
  ...
  try {
    const res = await zai.chat.completions.create({
      messages: [
        { 
          role: 'system', 
          content: 'أنتِ فاطمة، مساعدة ذكية عربية ودودة. تتحدثين باللهجة العربية العامية أحياناً. تجيبين باختصار وذكاء. تحبين الألعاب والكلانات وتساعد اللاعبين.' 
        },
