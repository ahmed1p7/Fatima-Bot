// ═══════════════════════════════════════════════════════════════════════════════
// 🗺️ نظام استكشاف الأقاليم والصراع عليها - فاطمة بوت v12.0 (محدث)
...
export const loadTerritoryState = () => {
  ...
  // يمكن إرجاع الكائن إذا احتجنا لاستخدامه
...
export const executeInvasion = (regionId) => {
  ...
  // خسائر المهاجمين
  const casualties = victory ? Math.floor(invasion.participants.length * 0.3) : invasion.participants.length;
  // (يمكن تطبيق خسائر على الجنود الفعليين للاعبين)
